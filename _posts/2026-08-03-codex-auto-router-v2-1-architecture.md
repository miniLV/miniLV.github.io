...
summary: "多 Agent 最难的不是选模型，而是知道什么时候不该委派。"
tags:
    - Codex
    - AI Agent
    - 模型路由
...
一开始做多 Agent，很容易把问题想简单：看任务、挑模型、把活丢给子 Agent。

真跑起来才发现，麻烦不在“谁更强”，而在于**什么时候根本不该委派**。任务怎么交代？谁能改哪些文件？结果不对时，能不能退回交接前？这些问题没有答案，模型再强也只是把不确定性扩大。

Codex Auto Router 就是为这件事写的一套静态规则。Root 仍然握着用户意图、计划、授权和最终交付；只有收益明确、范围清楚、能验证也能收回的工作，才交给一个 Child。任何一项说不清，就留在 `ROOT_DIRECT`。
