...
调用 Jev 需要 TypeSafe 账号与 API key，可按[官方 Quick Start](https://docs.typesafe.ai/introduction/quickstart)从控制台获取。当前仓库的代理读取环境变量 `JEV_API_KEY`；TypeSafe 官方示例使用 `TYPESAFE_API_KEY`，两者可以设置为同一密钥。不要把密钥写进仓库。

本地验证还需要 Node.js 22+、已登录的 Codex CLI，以及宿主实际可请求的模型与推理档位。拥有密钥并不等于逐调用代理已能接入真实 Codex 会话；后面的 P0 实验必须先通过。
...
3. **执行门禁只校验，不改选。** 确定性代码检查 Jev 返回的组合、权限和当前状态，结果只有 ALLOW 或 DENY。失败、低置信或 DENY 走有记录的预设回退；关闭路由则保留宿主原本指定的模型。
4. **原生执行并记录真实结果。** Responses 调用保持 Codex 的会话与工具循环，流式事件原样返回。日志记录实际执行的模型、档位、用量和回退原因；未观测到的用量保留为 `UNKNOWN`。

这套设计没有「Sol 失败就自动升 GPT-6」的第二个选路器，也不通过任务类型表暗中替 Jev 作决定。Jev 选的是已经合法的候选，门禁验的是这次具体选择能否执行。
...
本地代码和当前设计见 [Jev Auto Router 仓库](https://github.com/miniLV/Jev-Auto-Router)、[ADR 0017](https://github.com/miniLV/Jev-Auto-Router/blob/master/docs/adr/0017-per-call-responses-routing.md) 与[完整架构方案](https://github.com/miniLV/Jev-Auto-Router/blob/master/docs/solution.md)。在 P0 和受控对照完成前，我不会声称它已经能开箱即用，或已经证明普遍节省。
