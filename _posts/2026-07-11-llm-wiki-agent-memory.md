...
author:     "miniLV"
header-img: "img/headers/2026-07-11-llm-wiki-agent-memory.png"
summary:    "Coding Agent Memory 把本机的 Codex、Claude Code 会话整理成可查询、可审查的 Markdown Wiki，让 Agent 从过去的工作中积累经验。"
tags:
    - AI
    - Agent
    - Obsidian
...
于是有了 [Coding Agent Memory](https://github.com/miniLV/coding-agent-memory)：把一次性的对话整理成留在本地、随着使用不断生长的工程知识库。

<br>

## 怎么把对话变成能用的知识

第一步不是让 Agent “记住一切”，而是把当天真正发生过的事情留下来。

系统读取本机的 Codex 和 Claude Code session，把对话整理成一页 Daily，记录任务目标、调查过程和最终改动，并保留原始会话的证据链接。

Daily 只是一份工作备份，不等于长期知识。真实对话里有试错、重复命令和中途被推翻的猜测，第一次总结出来的“经验”也可能只在当前任务成立。

因此还需要第二次 Review。系统会复查一段时间内的 Daily，只有证据充分、可以指导下一次行动的内容才会进入 Concept。其余内容继续留在 Daily 里作为历史记录。

![Coding Agent Memory 架构图](/img/in-post/llm-wiki-agent-memory/agent-memory-arch-sketch.png)

当下一次任务遇到相似问题时，loader 会按需找回相关 Daily 和 Concept。这里的“自我进化”不是让模型自己修改规则，而是从真实工作中积累证据，经过复查后再复用。
...
![Coding Agent Memory 本地配置界面](/img/in-post/llm-wiki-agent-memory/local-config-ui.png)
...
[GitHub：miniLV/coding-agent-memory](https://github.com/miniLV/coding-agent-memory)
